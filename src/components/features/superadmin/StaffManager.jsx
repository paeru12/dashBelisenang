...
export default function StaffManager() {
  ...
  const { data: res, mutate } = useSWR(
    ...
    () => getUserPagination({ page, perPage, search })
  );

  // FIX STRUCTURE SESUAI API BARU
  const mediaBase = res?.data?.media || "";
  const raw = res?.data?.data || []; // array user dari API

  // Format ulang data agar cocok untuk DataTables
  const data = raw.map((item) => ({
    ...
    image: item.user?.image
      ? `${mediaBase}${item.user.image.replace(/^\/+/, "")}`
      : null,
  }));
      ...
      <DataTables
        ...
        columns={[
          ...
          },
          {
            ...
            label: "Action",
            render: (row) => {
              // Jika SUPERADMIN → sembunyikan tombol aksi
